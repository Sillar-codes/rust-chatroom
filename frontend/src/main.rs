        ...
        if let Some(ws_msg) = &**ws_message {
            ...
            match websocket_message.message_type {
                ...
                WebSocketMessageType::UsersList => {
                    let users = websocket_message.users.expect("Missing user list payload");
    ...
    html! {
        <div class="container my-5">
            <div class="row">
                ...
                <div class="col-9">
                    <MesasgeList messages={messages} />
                </div>
            </div>
            <div class="row mt-4">
                <SendDialog change_username_callback={change_username_callback} send_message_callback={send_message_callback} username={username} />
            </div>
        </div>
    }
}
